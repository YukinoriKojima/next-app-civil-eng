  ...
  const bidResultButton: MouseEventHandler<HTMLButtonElement> = async () => {
    ...
    for (let num = 0; num < 5; num++) {
      ...
      for (let player = 0; player < 4; player++) {
        if (
          Number(tmpResults[player][num]) <= upperCost[num] &&
          Number(tmpResults[player][num]) >= lowerCost[num]
        ) {
          if (
            tmp <
            (maxBid[num] * 10) / Number(tmpResults[player][num]) +
              areaPoint[tmpAreaList[player]][bidArea[num]]
          ) {
