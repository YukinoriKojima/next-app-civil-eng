    ...
    const bidResultButton: MouseEventHandler<HTMLButtonElement> = async () => {
        ...
        for (let num = 0; num < 5; num++) {
            ...
            for (let player = 0; player < 4; player++) {
                if (Number(results[player][num]) <= upperCost[num] && Number(results[player][num]) >= lowerCost[num]) {
                    ...
                    else if (tmpFirst < Number(results[player][num]) && Number(results[player][num]) < tmpSecond){
